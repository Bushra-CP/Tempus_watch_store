document.querySelectorAll('.removeOffer').forEach((btn) => {
  btn.addEventListener('click', async (e) => {
    ...
    if (confirmation.isConfirmed) {
      try {
        const response = await fetch('/admin/productOffers/removeOffer', {
          method: 'DELETE',
...
document.querySelectorAll('.deactivateOffer').forEach((btn) => {
  btn.addEventListener('click', async (e) => {
    ...
    if (confirmation.isConfirmed) {
      try {
        const response = await fetch('/admin/productOffers/deactivateOffer', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ offerId }),
        });
...
document.querySelectorAll('.activateOffer').forEach((btn) => {
  btn.addEventListener('click', async (e) => {
    ...
    if (confirmation.isConfirmed) {
      try {
        const response = await fetch('/admin/productOffers/activateOffer', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ offerId }),
        });
